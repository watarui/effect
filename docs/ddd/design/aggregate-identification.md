...
1. **UserItemRecord と ItemLearningRecord の重複**

   - 両方とも「ユーザー × 項目」の学習状態を管理
   - 統合の検討が必要かもしれない

2. **Progress Context の特殊性**

   - 唯一集約を持たないコンテキスト
   - これは設計意図通りだが、他のコンテキストとの一貫性に欠ける

3. **VocabularyEntry の複雑さ**
   - 内部に複数の VocabularyItem を持つ
   - Wikipedia スタイルは適切だが、実装が複雑になる可能性
