...
use shared_database::Config as DatabaseConfig;
use tracing::info;
...
pub async fn start(config: ServiceConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_pool = shared_database::create_pool(&db_config).await?;

    // Redis クライアントを作成
    let cache_client = shared_cache::Client::new(&config.redis_url).await?;
