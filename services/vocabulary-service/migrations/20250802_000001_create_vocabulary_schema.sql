...
-- 制約を別途追加
ALTER TABLE vocabulary_items ADD CONSTRAINT check_status
CHECK (status IN ('draft', 'pending_ai', 'published'));
ALTER TABLE vocabulary_items ADD CONSTRAINT check_created_by_type
CHECK (created_by_type IN ('user', 'system', 'import'));

-- 部分インデックスで UNIQUE 制約を実装
CREATE UNIQUE INDEX vocabulary_items_unique_disambiguation
ON vocabulary_items (spelling, disambiguation)
